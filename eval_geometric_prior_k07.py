"""Geometric Prior Pruning eval: k=0.7, libero_object, 50 eps/task."""
import os
import sys
import json
import time
...
def run_eval():
    ...
    for task_id in range(min(n_tasks, 10)):
        ...
        results_per_task[str(task_id)] = {
            ...
            "success_rate": sr,
            "mean_steps": np.mean([MAX_STEPS if not s else 0 for s in task_successes]),
        }
        total_success += sum(task_successes)
        total_eps += len(task_successes)
        print(f"  Task {task_id} SR: {sr:.0%} ({sum(task_successes)}/{len(task_successes)})")
    ...
    out_path = PROJECT_DIR / "eval_results" / "eval_geometric_prior_k07.json"
