"""Compare our manual forward pass vs official predict_action."""
import os, io, math, json, sys, shutil
...
# Step 1: Copy OFT modeling files into checkpoint (what check_model_logic_mismatch does)
...
# Step 2: Update config.json auto_map  
...
print("Updated config.json auto_map")

# Step 3: Add OFT repo to path for prismatic imports
sys.path.insert(0, OFT_REPO)

# Step 4: Register and load model
...
# Set num_images_in_input (what official code does)
...
# Load action head & proprio projector
...
proprio_proj = ProprioProjector(llm_dim=llm_dim, proprio_dim=PROPRIO_DIM)
proprio_proj.load_state_dict(load_sd(pp_files[-1]))
proprio_proj = proprio_proj.to(DEVICE, dtype=torch.bfloat16).eval()
...
with torch.inference_mode():
    ...
    # Call official predict_action
    action, action_hidden = model.predict_action(
        **inputs,
    ...
    print(f"\n=== OFFICIAL predict_action output ===")
