"""Compare OFT model outputs with and without bidirectional attention."""
...
# Test 1: Causal attention (default)
print("\n=== TEST 1: CAUSAL ATTENTION (default) ===")
with torch.inference_mode():
    actions_causal, _ = model.predict_action(
        **inp1, unnorm_key=unnorm_key, do_sample=False,
        proprio=proprio_normed, proprio_projector=pp, action_head=action_head,
    )
print(f"Actions (causal): {actions_causal[0][:4]}...{actions_causal[0][-1]}")
print(f"Action chunk shape: {len(actions_causal)} x {len(actions_causal[0])}")

# Apply bidirectional patch
print("\n=== Applying bidirectional attention patch ===")
llm = model.language_model
for layer in llm.model.layers:
    layer.self_attn.is_causal = False

original_update_causal_mask = llm.model._update_causal_mask

def _no_causal_mask(self, attention_mask, input_tensor, cache_position, past_seen_tokens=None):
    return None

llm.model._update_causal_mask = _no_causal_mask.__get__(llm.model, type(llm.model))

# Verify the patch
print(f"  is_causal on layer 0: {llm.model.layers[0].self_attn.is_causal}")

# Test 2: Bidirectional attention
print("\n=== TEST 2: BIDIRECTIONAL ATTENTION (patched) ===")
with torch.inference_mode():
    actions_bidir, _ = model.predict_action(
        **inp1, unnorm_key=unnorm_key, do_sample=False,
        proprio=proprio_normed, proprio_projector=pp, action_head=action_head,
    )
print(f"Actions (bidir): {actions_bidir[0][:4]}...{actions_bidir[0][-1]}")

# Compare
print("\n=== COMPARISON ===")
diff = np.abs(np.array(actions_causal) - np.array(actions_bidir))
print(f"Max diff: {diff.max():.6f}")
print(f"Mean diff: {diff.mean():.6f}")
print(f"Are they identical? {np.allclose(actions_causal, actions_bidir, atol=1e-5)}")

for i in range(len(actions_causal)):
    d = np.abs(np.array(actions_causal[i]) - np.array(actions_bidir[i]))
    print(f"  Chunk {i}: max_diff={d.max():.6f}, causal={actions_causal[i][:3]}, bidir={actions_bidir[i][:3]}")
