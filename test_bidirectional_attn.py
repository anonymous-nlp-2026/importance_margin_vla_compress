"""Test OFT eval with bidirectional attention monkey-patch (simulating the custom transformers fork)."""
...
# ============================================================
# MONKEY-PATCH: Disable causal attention (simulate the OFT fork)
# ============================================================
print("Applying bidirectional attention monkey-patch...")
llm = model.language_model

# 1. Set is_causal=False on all attention layers
for layer in llm.model.layers:
    layer.self_attn.is_causal = False

# 2. Override _update_causal_mask to return None (no causal mask)
def _no_causal_mask(self, *args, **kwargs):
    return None
llm.model._update_causal_mask = _no_causal_mask.__get__(llm.model)

# Verify
for i, layer in enumerate(llm.model.layers):
    if i == 0:
        print(f"  Layer 0 is_causal: {layer.self_attn.is_causal}")
print(f"  _update_causal_mask patched: True")
...
# Use benchmark-style task description
