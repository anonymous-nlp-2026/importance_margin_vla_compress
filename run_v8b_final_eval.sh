...
CKPT=checkpoints/imm_anchor_v8b_twostage/final
CONFIG=configs/imm_anchor_v8b_twostage.yaml
VERSION=v8b
BASELINE_LOSS=0.004077

if [ ! -d "$CKPT" ] && [ ! -f "$CKPT" ]; then
    echo "ERROR: Checkpoint not found: $CKPT"
    echo "Stage 2 training may not be complete yet."
    echo "Check: ls checkpoints/imm_anchor_v8b_twostage/"
    exit 1
fi
...
CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH=. python3 /tmp/_eval_train_mode.py \
